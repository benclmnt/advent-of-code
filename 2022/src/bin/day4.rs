fn part_1(data: &[Vec<Vec<u8>>]) -> u32 {
    data.iter().map(|v| is_fully_contained(v)).sum::<u32>()
}

/// is_fully_contained returned 1 if any of the ranges is contained in the other range and 0 otherwise
fn is_fully_contained(ranges: &[Vec<u8>]) -> u32 {
...
fn overlaps(ranges: &[Vec<u8>]) -> u32 {
    1 - ((ranges[0][0] > ranges[1][1]) || (ranges[0][1] < ranges[1][0])) as u32
}

fn part_2(data: &[Vec<Vec<u8>>]) -> u32 {
    data.iter().map(|v| overlaps(v)).sum::<u32>()
}
